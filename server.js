...
app.post("/api/menu/refresh", asyncHandler(async (req, res) => {
  ...
  // Runs after the response above has already been sent — this is the part
  // that actually takes a few minutes, now decoupled from the HTTP request.
  try {
    ...
    scrapeState = {
      inProgress: false,
      error: "Scrape failed. Check server logs. You can still add custom foods manually.",
